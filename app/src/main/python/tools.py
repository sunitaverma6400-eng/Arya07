...
def _dispatch_phone_tool(tool_name: str, args: dict, local_check_bin: str, timeout: int = 20):
    """
    Jarvis teen jagah chal sakta hai: (1) native Android app ke andar
    (Chaquopy + native_android.py — ab yehi primary path hai), (2) seedha
    phone par Termux ke andar, (3) cloud (Render) par, jahan asli phone
    hardware nahi hota.

    Ye function decide karta hai konsa case hai:
    - Agar native_android module available hai (matlab hum Chaquopy-based
      Android app ke andar chal rahe hain), seedha usi se native Android
      API call karta hai — koi Termux ya phone_bridge polling nahi chahiye.
    - Warna agar `local_check_bin` (jaise 'termux-torch') isi machine par
      milta hai, matlab hum khud Termux ke andar hain — None return karta
      hai, taaki caller function apna normal (purana) local-execution wala
      code chalaye.
    - Warna (matlab cloud par hain), phone_bridge ke through connected
      phone-agent ko command bhejta hai aur result wait karta hai.
    """
    try:
        import native_android
        if native_android.available():
            native_func = getattr(native_android, tool_name, None)
            if native_func:
...
def web_search(query: str):
    """Tavily se web search — DuckDuckGo hataya gaya hai (kaam nahi kar raha tha).
    Tavily API key zaroori hai ('Jarvis code api: tavily TERI_KEY' bolkar save karo)."""
...
def search_images(query: str, count: int = 4):
    ...
    Priority: icrawler (Google/Bing crawler, no-key) → Serper.dev (agar key
    saved hai, asli Google Images SERP data) → Internet Archive (last resort).

    Google Custom Search, SerpApi, Brave, Bing (direct scrape), DuckDuckGo,
    Yandex, Startpage, Wikimedia Commons, Pixabay, Unsplash, Flickr, aur
    Openverse — yeh sab providers hata diye gaye hain (kaam nahi kar rahe
    the / unreliable the).

    Har provider ke result ke URLs dead-link-check (parallel HEAD request)
    se validate hote hain — koi bhi provider agar broken/dead links de to
    seedha agle provider pe fall-through ho jaata hai, user ko kabhi broken
    image icon nahi dikhta.
    """
    for fn in (_search_images_icrawler, _search_images_serper,
...
def _auto_route_video_lines(raw_result: str):
    """
    HLS player poori tarah hata diya gaya hai, isliye ab yeh sirf raw_result
    ko as-is waapas karta hai (koi HLS_FOUND conversion nahi hoti). Function
    naam call-sites ke saath compatibility ke liye rakha gaya hai.
    """
    return raw_result
...
def find_and_play(query: str, source: str = "auto") -> str:
    ...
    hoti hai:
      ...
      - "web" → PURI movie/TV episode, sports match highlights/live,
        ...
        specific) — web_search se best matching webpage dhoondh ke
        uspar get_page_media() se playable video nikala jaata hai
        (HLS player/generic resolver hataya ja chuka hai).
      - "auto" (default, jab type clear na ho) → pehle YouTube-priority
        search try hota hai, na mile to web search fallback.

    Pipeline:
    1. (source="youtube"/"auto") search_videos(query) — multi-source hai
       (YouTube API, yt-dlp, YouTube scrape, Invidious, Archive.org).
       Top result seedha play hota hai, baaki 2 bhi carousel mein swipe
       karke dekh sakte ho.
    2. (source="web"/"auto" agar step 1 khaali ho) web_search se top 2
       candidate links liye jaate hain aur unpar get_page_media() se
       playable video nikalne ki koshish hoti hai (max 2 attempts).
    """
    query = (query or "").strip()
    if not query:
    ...
    def _try_youtube_priority():
        ...
        if result and "nahi mile" not in result and "VIDEO_FOUND:" in result:
            return f"🔎 '{query}' dhoondh ke play kar raha hoon (aur options bhi neeche hain, swipe karo):\n{result}"
        return None

    def _try_web_priority():
        urls = re.findall(r"🔗\s*(\S+)", web_search(query) or "")
        tried = 0
        for candidate in urls:
            ...
            if not candidate.startswith("http"):
                continue
            tried += 1
            media = get_page_media(candidate, media_type="video", limit=1)
            if media and "VIDEO_FOUND:" in media:
...
def search_videos(query: str, count: int = 3):
    """
    Real videos dhundo — YouTube-priority chain (YouTube Data API → yt-dlp
    → YouTube direct scrape → Invidious), phir Internet Archive last
    resort ke taur par. Videos in-app embed se play honge.

    DuckDuckGo, Bing, Wikimedia (video sources) aur HLS player poori
    tarah hata diye gaye hain (kaam nahi kar rahe the).
    """
    # 1. YouTube Data API — agar user ne apni key di hai, sabse reliable
...
def get_page_media(url: str, media_type: str = "all", limit: int = 6) -> str:
    ...
    hai (IMAGE_FOUND / VIDEO_FOUND markers se).
    ...
    VIDEO EXTRACTION: page ke HTML se video links regex-scrape (video/
    source tags, og:video, JSON-LD) se nikale jaate hain. JS-heavy
    streaming/embed players jahan video sirf JavaScript se load hoti hai,
    unpar yeh kam kaam karega (HLS/generic yt-dlp resolver pipeline
    poori tarah hataya ja chuka hai).
    ...
    if low_path.endswith(_VID_EXTS):
        return f"🎬 Link se seedha video mil gaya.\nVIDEO_FOUND:{url}|Video"

    lines = []
    video_marker = None

    # Page ka HTML scrape karo, taaki (a) images mil sakein, aur (b) video
    # links mil sakein. Pehle normal fetch, block ho to cloudscraper
    # (stealth) try karta hai — taaki basic anti-bot (Cloudflare) wali
    # sites bhi khul sakein. (HLS player/yt-dlp generic extractor pipeline
    # poori tarah hataya ja chuka hai — ab sirf regex-scrape se video milta
    # hai.)
    body, err, used_stealth = _http_get_stealthy(url, headers={"Accept": "text/html"}, timeout=20, max_bytes=3_000_000)
