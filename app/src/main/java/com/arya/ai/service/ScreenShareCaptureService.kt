...
class ScreenShareCaptureService : Service() {
    ...
    private fun startCapture() {
        ...
        captureJob = scope.launch {
            // Same fix as CameraFrameCapture (see chat history) — capture soon after start
            // instead of only after the first full interval, so VisionFrameProvider isn't
            // empty for the first few seconds after screen-share turns on.
            delay(500)
