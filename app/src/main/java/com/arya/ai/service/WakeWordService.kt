...
class WakeWordService : Service(), RecognitionListener, TextToSpeech.OnInitListener {
    ...
    private fun speak(text: String, emotion: String = com.arya.ai.util.AvatarEmotion.NEUTRAL, thenListenForCommand: Boolean = false) {
        ...
        scope.launch {
            try {
                val (reply, emotion) = when {
                    ...
                    visionFrame != null ->
                        (com.arya.ai.tools.VisionRelay.describeImage(visionFrame, command)?.let { it to com.arya.ai.util.AvatarEmotion.NEUTRAL }
    ...
    private suspend fun generateTextOnlyReply(command: String): Pair<String, String> {
        ...
        val result = withContext(Dispatchers.IO) {
            com.arya.ai.util.OnlineChatHelper.generateOnlineResponse(prefs, command, systemPrompt)
        }
