...
fun SettingsScreen(onBack: () -> Unit) {
    ...
    var speakChatReplies by remember { mutableStateOf(prefs.ttsEnabled) }
    ...
    Scaffold(
        ...
        Column(
            ...
            Divider()
            SettingsRow(
                title = "Bolke jawab do",
                subtitle = "Typed chat replies bhi ElevenLabs ki emotional voice me bolo — mood (khush/udaas/serious " +
                    "waghera) reply ke hisaab se apne aap badalta hai. Voice/\"Hey Arya\" replies already bolti hai, " +
                    "yeh sirf normal type-karke-chat wale screen ke liye hai.",
                checked = speakChatReplies,
                onCheckedChange = { speakChatReplies = it; prefs.ttsEnabled = it }
            )
            Divider()
            SettingsRow(
