...
data class OnlineChatResult(val text: String, val providerUsed: String, val modelUsed: String)
...
object OnlineChatHelper {
    ...
    fun generateOnlineResponse(
        ...
        for (provider in providerOrder) {
            ...
            for (model in orderedModels) {
                try {
                    val text = callRelay(relayUrl, provider, model.id, prompt, systemPrompt)
                    return OnlineChatResult(text, provider.label, model.id)
                } catch (e: Exception) {
    ...
    fun streamOnlineResponse(
        ...
        for (provider in providerOrder) {
            ...
            for (model in orderedModels) {
                try {
                    val text = callRelayStream(streamUrl, provider, model.id, prompt, systemPrompt, onChunk)
                    return OnlineChatResult(text, provider.label, model.id)
                } catch (e: Exception) {
                    lastError = e // try next model, then next provider
                }
    ...
    private fun callRelayStream(
        ...
        // NDJSON: one {"delta"|"done"|"error": ...} object per line — see the relay's
        // /v1/relay/stream doc comment for the exact contract.
    ...
    private fun callRelay(
        ...
        return JSONObject(responseText).getString("text")
    }
}
