...
class PreferencesManager(context: Context) {
    ...
    /** Stable per-install identifier used only for relay rate limiting. It is random, contains
     * no account/device information, and is regenerated only if app data is cleared. */
    val relayClientId: String
        get() {
            val existing = prefs.getString("relay_client_id", null)
            if (!existing.isNullOrBlank()) return existing
            val created = UUID.randomUUID().toString()
            prefs.edit().putString("relay_client_id", created).apply()
            return created
        }
    ...
    /** Canvas tile's scratchpad draft (FIXES_LOG.md Phase 26) — persisted so leaving the
     *  Canvas screen and coming back (or reopening the app) doesn't lose unsent text. */
    var canvasDraft: String
        get() = prefs.getString("canvas_draft", "") ?: ""
        set(value) = prefs.edit().putString("canvas_draft", value).apply()

    /** "Personal Intelligence" tile (FIXES_LOG.md Phase 26) — when [personalIntelligenceEnabled]
     *  is true, [personalContext] gets appended to every system prompt (see
     *  MainActivity.buildIdentityContext) so Arya's replies can take it into account. Off by
     *  default — this is opt-in personalization, not something silently switched on. */
    var personalIntelligenceEnabled: Boolean
        get() = prefs.getBoolean("personal_intel_enabled", false)
        set(value) = prefs.edit().putBoolean("personal_intel_enabled", value).apply()

    var personalContext: String
        get() = prefs.getString("personal_context", "") ?: ""
        set(value) = prefs.edit().putString("personal_context", value).apply()
