...
object ImageTools {
    ...
    /**
     * Same Pollinations endpoint as [generateImage], but downloads and decodes it into a real
     * [android.graphics.Bitmap] instead of just returning the URL as text — used as the
     * free/keyless fallback in [AryaToolRegistry]'s `generate_image` when the relay isn't
     * configured or fails, so the caller can attach/save it exactly like the relay path.
     */
    fun fetchGeneratedBitmap(prompt: String): android.graphics.Bitmap? {
        return try {
            val url = "https://image.pollinations.ai/prompt/${enc(prompt)}?width=768&height=768&nologo=true"
    ...
    /** Downloads a [searchImages] result URL and decodes it into a [android.graphics.Bitmap] —
     *  used by [AryaToolRegistry]'s `search_images` branch so results actually show up as
     *  pictures in chat (via the same `lastSearchedImages` side-channel [generateImage] uses
     *  for [AryaToolRegistry.lastGeneratedImage]), not just text URLs the model describes.
     *  Smaller timeout than [fetchGeneratedBitmap] since these are thumbnails, not full
     *  generations — a slow/broken result shouldn't stall the whole tool call. */
    fun fetchThumbnail(url: String): android.graphics.Bitmap? {
        return try {
            val conn = (URL(url).openConnection() as HttpURLConnection).apply {
    ...
    fun testImageSource(url: String): String {
        ...
        return if (code in 200..299 && type.startsWith("image/")) "✅ Image source valid hai ($type, HTTP $code)"
