...
object InfoApiTools {
    ...
    fun getWeather(city: String): String {
        ...
        val result = geo?.optJSONArray("results")?.optJSONObject(0)
            ?: return "❌ '$city' नहीं मिला"
        ...
        )?.optJSONObject("current") ?: return "❌ $name ka weather data nahi mila"
    ...
    fun getCryptoPrice(coin: String): String {
        ...
        val json = NetTools.getJson("https://api.coingecko.com/api/v3/simple/price?ids=$id&vs_currencies=usd,inr")
            ?.optJSONObject(id) ?: return "❌ '$coin' coin nahi mila"
        val usd = json.optDouble("usd")
        val inr = json.optDouble("inr")
        return "💰 ${coin.uppercase()}: \$$usd USD / ₹$inr INR"
    }

    fun convertCurrency(amount: Double, from: String, to: String): String {
        val json = NetTools.getJson("https://open.er-api.com/v6/latest/${from.uppercase()}")
            ?: return "❌ Currency rates fetch nahi ho paaye"
        val rate = json.optJSONObject("rates")?.optDouble(to.uppercase())
            ?: return "❌ '$to' currency nahi mili"
