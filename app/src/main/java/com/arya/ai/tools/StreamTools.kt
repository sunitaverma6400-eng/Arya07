...
object StreamTools {
    ...
    private fun searchVideosGeneric(url: String, query: String, maxResults: Int, mustContain: String?): String {
        ...
        return try {
            ...
            "🎬 Videos for '$query' (browser/app me kholna hoga, direct play nahi):\n" + links.joinToString("\n") { "• $it" }
        } catch (e: Exception) {
    ...
    fun testVideoSource(url: String): String = ImageTools.testImageSource(url)
        .let { if (it.startsWith("✅")) "✅ URL reachable hai" else it.replace("image", "video source") }
