...
object ImageGenTools {
     ...
     *  Retries once on transient failure (relay cold-start on Render free tier is a common
     *  cause of the first attempt timing out) before giving up. */
    suspend fun generate(prompt: String): Bitmap? = withContext(Dispatchers.IO) {
        ...
        repeat(2) { attempt ->
            val bitmap = tryGenerate(base, prompt)
    ...
    private fun tryGenerate(base: String, prompt: String): Bitmap? {
        return try {
