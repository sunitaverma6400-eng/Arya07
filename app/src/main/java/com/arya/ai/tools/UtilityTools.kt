...
object UtilityTools {
    ...
    /** Bitmap-returning variant of [generateQr] for direct Compose display (e.g. showing a
     *  Family Circle code as a QR) — same ZXing encode, just skips the file-write since the
     *  UI doesn't need a saved file, only the image. */
    fun generateQrBitmap(text: String, size: Int = 400): Bitmap? = try {
