...
## 13 Aug 2026 hardening pass

This archive includes relay key cooldowns, chat request limits, request IDs, improved Gemini
routing/vision model selection, a per-install relay client ID, CI configuration, and relay
regression tests. The environment used to prepare this archive did not contain the Android SDK
or Gradle distribution, so a real APK build still needs to be run on a machine/CI runner with
